...
FnAttribute.prototype = {
    constructor: FnAttribute,
    setBack: function() { //把区域分成一个一个的小方块。
        for (var i = 0; i < 30; i++) {
            for (var j = 0; j < 30; j++) {
    ...
    fnMoveup: function() { //上下运动，取第二位的字符转化为数字，如果为数字，则后面造字符串的时候从第二位开始，否则从第一位开始。
    ...
    fnMovedown: function() { //上下运动，取第二位的字符转化为数字，如果为数字，则后面造字符串的时候从第二位开始，否则从第一位开始。
        var oNowDiv = document.getElementById("div2");
        this.sPreParentId = oNowDiv.parentNode.id;
        var nIf = parseInt(this.sPreParentId.slice(1, 2));
        if (isNaN(nIf)) {
    ...
    fnEatFood: function() { //吃食物的函数，如果蛇头和食物的父div的id相同，则删除当前的食物。
    ...
    fnFollowDiv: function() { //后长出来的身体的移动的函数。思路就是一直把后一个div放到起一个div的父节点里面。
